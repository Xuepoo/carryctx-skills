...
| Correct terminal task | `carryctx task edit CTX-0002 --title "..." --force`                                                                    | Explicit audited authorization required for terminal-task corrections          |
...
| Worktree cleanup      | `carryctx worktree cleanup list\|show <REF>\|run [--dry-run]`                                                          | Durable outbox; run applies retryable policy and dry-run changes nothing       |
| Worktree remove       | `carryctx worktree remove <TASK_REF\|PATH> [--force]`                                                                  | Direct removal; prefer cleanup policy for completed/cancelled tasks            |
